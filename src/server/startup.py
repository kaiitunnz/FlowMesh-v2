...
from .ingress import InferenceIngress
...
async def rehydrate_root_state(
    ...
    inference_ingress: InferenceIngress | None = None,
) -> None:
    ...
    an empty store and strand (or re-admit a second) credit. Ingress terminals replay
    after the claim store loads so a claim left UNCERTAIN by a crash between its
    ...
    if resident_control is not None and resident_registry is not None:
        ...
        if inference_ingress is not None:
            inference_ingress.reconcile_terminals()
