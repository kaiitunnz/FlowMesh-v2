...
class ServeIngressLane:
    ...
    def on_stream_head(self, head: ResidentStreamHead) -> None:
        # The head is delivered to the client's channel; the response is marked
        # committed only once the listener writes it to the client socket (the channel's
        # commit hook), never here on mere receipt, so a loss before the write may still
        # recover.
