...
- **Task merge** coalesces tasks whose specs share a merge key into one dispatch.
  The worker receives a parent payload with `merged_children`, the executor runs
  every task at once and returns each child's result under `result.children`, and
  the runner stores each child's result on its own. A child that gets no result
  runs again alone. Disabled with `ENABLE_TASK_MERGE=false` on the server.
