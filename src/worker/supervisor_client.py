...
class SupervisorClient:
    ...
    def push_serve_ingress_register(self, advertisement: dict[str, Any]) -> None:
        """Advertise this worker's forward serve ingress to control for registration."""
        if self._stub is None:
            raise RuntimeError("Supervisor gRPC client not started")
        if not self._event_ready.wait():
            raise RuntimeError("Supervisor event stream not ready")
    ...
    def push_serve_ingress_request(self, request: dict[str, Any]) -> None:
        """Ask control to authenticate, authorize, and admit one forward-ingress
        request."""
        if self._stub is None:
            raise RuntimeError("Supervisor gRPC client not started")
        if not self._event_ready.wait():
            raise RuntimeError("Supervisor event stream not ready")
        event = WorkerEvent(
            type="SERVE_INGRESS_REQUEST",
            worker_id=self.worker_id,
            payload={"request": request},
        )
        self._event_queue.put(serialize_event(event))

    def push_serve_committed(
        self, invocation_id: str, status: int, headers: list[list[str]]
    ) -> None:
        """Report a forward-ingress response committed to its own client to control."""
        if self._stub is None:
            raise RuntimeError("Supervisor gRPC client not started")
        if not self._event_ready.wait():
            raise RuntimeError("Supervisor event stream not ready")
        event = WorkerEvent(
            type="SERVE_COMMITTED",
            worker_id=self.worker_id,
            payload={
