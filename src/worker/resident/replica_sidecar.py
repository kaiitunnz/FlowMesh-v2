...
class ResidentReplicaSidecar:
    ...
    async def on_frame(
        ...
        ``sink``, so the session it opens answers over the same connection rather than
        this worker's attachment.
        """
    ...
    def _engine_request_span(
        ...
        with _otel.get_tracer().start_as_current_span(
            SPAN_ENGINE_REQUEST, context=parent_context, attributes=attributes
        ) as span:
            yield span
