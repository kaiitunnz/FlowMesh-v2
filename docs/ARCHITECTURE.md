...
- **Reference-backed invocation outcomes.** A mediated boundary settles by reference: the
  ...
  [`EXECUTORS.md`](EXECUTORS.md).
- **Control-plane profiling.** A v2 submission's server-side control stages are timed and
  rolled up per workflow in the metrics recorder, reported through
  `GET /api/v1/system/metrics`. Each measurement carries the window it fires in — the
  submit request, a task's queue window, or mid-episode after the task started — and
  whether an enclosing stage already counts it. Since v1 and v2 share one process,
  dispatcher and worker pool, submitting the same body on both tracks isolates the v2
  control-plane cost. Enable with `SERVER_METRICS_CONTROL_PROFILING=true`. See
